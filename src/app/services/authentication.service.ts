...
export class AuthenticationService {
  ...
  storeTokensWithCodeAndReturnUser(code: string): Observable<Record<string, any>> {
    console.log(environment.keycloakClientID, environment.keycloakRedirectURI, environment.keycloakRCodeVerifier, code);
    const body: URLSearchParams = new URLSearchParams();
    body.set('grant_type', 'authorization_code');
    body.set('client_id', environment.keycloakClientID);
    body.set('client_secret', environment.keycloakClientSecret);
    body.set('code', code);
    body.set('redirect_uri', environment.keycloakRedirectURI);
    body.set('code_verifier', environment.keycloakRCodeVerifier);
    return this.httpClient.post<Record<string, any>>(`${KeycloakConfig.keycloakUrlForToken}`, body.toString()).pipe(
      ...
      tap((response: Record<string, any>) => {
        const userStringified: string = JSON.stringify(response as AuthUser);
        // TODO Tester si la key/value enregistre un string par défaut ou si il s'en fou.
        return this.storage.currentUser = JSON.parse(userStringified);
      }),
    );
  }
  ...
  getDecodedAccessToken(record: Record<string, any>): Observable<any> {
    try {
      if (record['access_token'] != null) {
        ...
        return throwError(() => new Error(ErrorEnum.noAccessToken));
      }
    } catch (e: any) {
  ...
  isTokenExpired(token: string): boolean {
    try {
      ...
      throw new Error(ErrorEnum.noAccessToken);
    }
  }
  ...
  checkUserIsLogged(): Observable<boolean> {
    ...
    return of(this.storage.accessToken).pipe(
      mergeMap((accessToken: string) =>
        iif((): boolean => accessToken != ErrorEnum.noAccessToken && !this.isTokenExpired(accessToken), tokenIsValid$, tokenNotIsValid$),
      ),
    );
  }
}
